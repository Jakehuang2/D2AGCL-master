...
class Metattack(BaseMeta):
    ...
    def attack(self, ori_features, ori_adj, labels, idx_train, idx_unlabeled, n_perturbations, ll_constraint=True,
               ll_cutoff=0.004):
        """对输入图生成n个扰动。
        ...
        idx_unlabeled:
            未标记节点的索引
        n_perturbations : int
            输入图上的扰动次数。扰动可以是边的删除/添加或特征的删除/添加。
        ll_constraint: bool
            是否施加似然比检验约束
        ll_cutoff : float
            似然比检验的临界值，用于幂律分布的Chi平方分布检验，具有1个自由度。默认值0.004对应的p值大约为0.95。
            如果`ll_constraint`为False，该参数将被忽略。
        """

        # 初始化存储扰动边、添加的边数和删除的边数的列表
        perturbed_edges = []
        edge_num_add = []
        edge_num_del = []
        ...
        for i in tqdm(range(n_perturbations), desc="Perturbing graph"):
            ...
            perturbed_edges.append((row_idx, col_idx))

        # 如果启用了结构攻击，最终使用扰动后的邻接矩阵
        if self.attack_structure:
            self.modified_adj = self.get_modified_adj(ori_adj).detach()

        # 如果启用了特征攻击，最终使用扰动后的特征矩阵
        if self.attack_features:
